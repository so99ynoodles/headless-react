    ...
    `}
        ...
        {({ selectedItem }) => (
          <>
            <span className="flex items-center">
              {selectedItem ? (
                <>
                  <img src={selectedItem.avatar} alt="" className="flex-shrink-0 h-6 w-6 rounded-full" />
      ...
      <Select.Popover>
        <Select.Options className="z-10 mt-1 w-full bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
          {({ options }) =>
            options.map((option) => option.type === 'section' ? ((
              <Select.Section key={option.key} section={option}>
                {({ section }) => (
                  <>
                    {section.rendered && <Select.SectionHeading className='text-xs font-bold uppercase text-gray-500 mx-3'>{section.rendered}</Select.SectionHeading>}
